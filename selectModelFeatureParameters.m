function [modelrpfile, modelrpidx, modelrunparams, validresponse] = selectModelFeatureParameters(fv1name)

% selectModelFeatureParameters - select the matlab saved variable file for the 
% combinations of parameters to create model features and labels for

[~, studydisplayname, ~] = selectStudy();

basedir = setBaseDir();
subfolder = 'DataFiles';
matchstring = sprintf('*pmmfp%s*%s*.xlsx', fv1name, studydisplayname);
...
fprintf('Model feature parameter files available\n');
fprintf('---------------------------------\n');
