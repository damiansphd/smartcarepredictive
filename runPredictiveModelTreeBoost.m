...
if ~ismember(basehpparamfile, '')
    ...
    [lrarray, lcarray, mlsarray, mnsarray, fvsarray, nlr, nlc, nmls, nmns, nfvs] = setHyperParameterArrays(pmHyperParams);
    
else
    lrarray = 1; lcarray = 1; mlsarray = 1; mnsarray = 1; fvsarray = 1; nlr = 1; nlc = 1; nmls = 1; nmns = 1; nfvs = 1;
end
hpsuffix = sprintf('lr%.2f-%.2flc%d-%dml%d-%dns%d-%dfv%.2f-%.2f', ...
                    lrarray(1),  lrarray(end),  lcarray(1),  lcarray(end), ...
...
for fs = 1:nfeatureparamsets
    ...
    for mp = 1:nmodelparamsets
        ...
        nhpcomb      = nlr * nlc * nmls * nmns * nfvs;
        ...
        hyperparamQS = table('Size',[nhpcomb, 16], ...
                        ...
                        'VariableNames', {'LearnRate', 'NumLearnCycles', 'MinLeafSize', 'MaxNumSplit', 'FracVarsToSample', ...
                ...
                for mls = 1:nmls
                    ...
                    for mns = 1:nmns
                        for fvs = 1:nfvs
                            ...
                            tic
                            mnsval = mnsarray(mns);
                            hpcomb = ((lr - 1) * nlc * nmls * nmns * nfvs) + ((lc - 1) * nmls * nmns * nfvs) + ((mls - 1) * nmns * nfvs) + ((mns - 1) * nfvs) + fvs;
                            ...
                            fprintf('LR: %.2f LC: %3d MLS: %3d MNS: %3d - Qual Scores: ', lrval, lcval, mlsval, mnsval);
                            ...
                            hyperparamQS.NumLearnCycles(hpcomb)   = lcval;
