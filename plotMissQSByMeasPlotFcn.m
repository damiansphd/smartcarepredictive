...
line(ax, [0 100], [100 * qsthreshold 100 * qsthreshold], ...
    'Color', 'black', ...
    'LineStyle', '-', ...
    'LineWidth', 1, ...
    'Marker', 'none');
line(ax, [0 100], [100 * fpthreshold 100 * fpthreshold], ...
    'Color', 'black', ...
    'LineStyle', ':', ...
    'LineWidth', 1, ...
    'Marker', 'none');
...
title(sprintf('QS:%s Meas:%s Outcome:%s', qsmeasure, meas, outcome), 'FontSize', labelfontsize);

if showlegend
    legend(sprintf('TN  (%d/%.1f%%)', sum(tnidx),  100 * sum(tnidx)  / size(tnidx,  1)), ...
           sprintf('FN  (%d/%.1f%%)', sum(fnidx),  100 * sum(fnidx)  / size(fnidx,  1)), ...
           sprintf('AFP (%d/%.1f%%)', sum(fp1idx), 100 * sum(fp1idx) / size(fp1idx, 1)), ...
           sprintf('BFP (%d/%.1f%%)', sum(fp2idx), 100 * sum(fp2idx) / size(fp2idx, 1)), ...
           sprintf('TP  (%d/%.1f%%)', sum(tpidx),  100 * sum(tpidx)  / size(tpidx,  1)), ...
           'Location',  'eastoutside');
end

xlim(ax, [0 100.5]);
ylim(ax, [0 max(ydata) + 0.5]);
