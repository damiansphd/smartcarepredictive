...
[pmInterpVolcube, mvolstats, ~] = createPMInterpVolcube(pmPatients, pmInterpDatacube, ...
    npatients, maxdays, nmeasures, pmFeatureParamsRow.featureduration, 4, ...
    pmFeatureParamsRow.normwindow); 
toc
fprintf('\n');
...
if plottype == 6
    ...
    pnbr = [pnbr1; pnbr2];
    plotMeasuresAndPredictionsForPatientForPaper3New(pmPatients(pnbr1,:), pmPatients(pnbr2,:),...
...
elseif plottype == 10
    ...
    plotMeasuresAndPredictionsForPatientForPaperNew(pmPatients(pnbr,:), ...
...
elseif plottype == 11
    ...
    fprintf('Plotting results for patient %d\n', pnbr);
    plotMeasuresAndPredictionsForPatientForPaper2New(pmPatients(pnbr,:), ...
        pmAntibiotics(pmAntibiotics.PatientNbr == pnbr & pmAntibiotics.RelStopdn >= 1 & pmAntibiotics.RelStartdn <= pmPatients.RelLastMeasdn(pnbr),:), ...
        pmAMPred(pmAMPred.PatientNbr == pnbr,:), ...
        pmRawDatacube(pnbr, :, :), pmInterpDatacube(pnbr, :, :), pmInterpVolcube(pnbr, :, :), ...
        testfeatidx, testlabels, pmModelRes, ...
