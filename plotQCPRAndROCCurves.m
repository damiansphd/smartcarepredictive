...
hold on;
area(ax, 1:size(pmQCModelRes.QCCostArray, 1), pmQCModelRes.QCCostArray, ...
    'FaceColor', 'blue', 'LineStyle', '-', 'LineWidth', 1.5);

line(ax, [pmQCModelRes.IdxOp, pmQCModelRes.IdxOp], [0, max(pmQCModelRes.QCCostArray)], ...
    'Color', 'red', 'LineStyle', '-', 'LineWidth', 1.0);

scatter(ax, rocthreshidx, pmQCModelRes.QCCostArray(rocthreshidx),  ...
        24, 'filled', 'o', ...
        'MarkerFaceColor', 'yellow', ...
        'MarkerEdgeColor', 'black');

scatter(ax, pmQCModelRes.IdxOp, pmQCModelRes.QCCostOp,  ...
        24, 'filled', 'o', ...
        'MarkerFaceColor', 'green', ...
        'MarkerEdgeColor', 'black');
...
ax.FontSize = axisfontsize; 
ax.TickDir = 'out';      
xlim(ax, [1 size(pmQCModelRes.QCCostArray, 1)]);
ylim(ax, [0 max(pmQCModelRes.QCCostArray)]);

xlabel(ax, 'Index Point');
ylabel(ax, 'QC Cost');

qccosttext = sprintf('Min Cost = %.5f', pmQCModelRes.QCCostOp);
annotation(p, 'textbox',  ...
                'String', qccosttext, ...
                'Interpreter', 'tex', ...
                'Units', 'normalized', ...
                'Position', [0.71, 0.7 0.125, 0.1], ...
                'HorizontalAlignment', 'center', ...
                'VerticalAlignment', 'middle', ...
                'BackgroundColor', 'white', ...
                'LineStyle', '-', ...
