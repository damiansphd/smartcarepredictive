...
for i = 1:(ntitles + nmeasures + npredictions + nlabels)
    ...
    elseif type == 4 || type == 5 || type == 6
        ...
        if currplot <= nmeasures
            ...
            if ~(measures.RawMeas(m) || measures.BucketMeas(m) || measures.Range(m) || measures.AvgSeg(m) || measures.CChange(m))
                smcolour = [0.73, 0.73, 0.73];
            end
            if ~(measures.Volatility(m) || measures.VolSeg(m))
            ...
            %plotMeasurementDataForPaper(ax, days(normwindow+2:end), smooth(vdata(normwindow+2:end),5), smcolour, '-', 1.5, 'none', 1.0);
            %plotMeasurementDataForPaper(ax, days(normwindow+2:end), intervppts(normwindow+2:end), right_color, 'none', 1.0, 'o', 1.0);
            ...
            %plotMeasurementDataForPaper(ax, days, mdata, rwcolour, '-', 1.5, 'none', 1.0);
            plotMeasurementDataForPaper(ax, days, applySmoothMethodToInterpRow(mdata, smfn, smwin, smln, m, mfev1idx), smcolour, '-', 1.5, 'none', 1.0);
            %plotMeasurementDataForPaper(ax, days, interppts, lint_color, 'none', 1.0, 'o', 1.0));
            ...
            %plotMeasurementDataForPaper(ax, days, plabeldata * 100, 'green', '-', 1.5, 'none', 1.0);
