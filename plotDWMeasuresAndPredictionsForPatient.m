...
plotsacross = 1;
if ismember(pmFeatureParamsRow.StudyDisplayName, 'BE')
    plotsdown = 6;
    npages      = ceil((sum(measures.RawMeas) + 1) / plotsdown);
else
    plotsdown   = 10;
...
for period = 1:nperiods
    ...
    for m = 1:nmeasures
        ...
        if (ismember(pmFeatureParamsRow.StudyDisplayName, 'BE') && measures.RawMeas(m) == 1)
